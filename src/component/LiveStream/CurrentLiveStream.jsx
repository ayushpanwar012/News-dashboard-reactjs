...
const CurrentLiveStream = () => {
  ...
  const validateCategory = () =>{
    ...
    const categoryFormUrl = queryParams.get("category");
    const storedCategories =JSON.parse(localStorage.getItem('live')|| '[]');

    if(storedCategories.length>0){
      console.log(storedCategories)
    }

    if(storedCategories.includes(categoryFormUrl)){
      console.log("Category from URL matched Access Array. Extracted Category from the URL is: ", categoryFormUrl)
      return categoryFormUrl
    }
    // fall back to first category from  userCategory array
    const firstCategory = storedCategories[0] || "null";
  ...
  const Fetch = useCallback(async (category) => {
    ...
    try {
      const api_link = `/api/fetch-concurrent-views?${queryParams.toString()}`;
      ...
      const channelName = [
        "Top 50 streams",
        "All channel top",
        ...new Set(sortedvideos.map((e) => e.channel_name)),
      ];
  ...
  useEffect(() => {
    ...
    if (selectedChannel === "Top 50 streams") {
      ...
      const newFilterVideos = videos.filter(
        (video) => video.channel_name === selectedChannel
      );
      setFilterVideos(newFilterVideos);
    }
    ...
    <div>
      ...
      <div>
        <h2 style={{ textAlign: "center" }}>
          {getCategory} Current LiveStreams
        </h2>
        <div
          style={{ textAlign: "center", margin: "20px", fontFamily: "Ariel" }}
        >
        ...
        <div className="tab-container">
          ...
          >
            <button
              className="Top"
              onClick={() => handleTabChange("topLivestream")}
            >
              Top LiveStream
            </button>
            <button
            ...
            <button
              className="Hash"
              onClick={() => handleTabChange("tagranking")}
            >
              Hash tag Ranking
            </button>
          </div>
          ...
          {activetab === "topLivestream" && (
            ...
            >
              {filtervideos.map((video, index) => (
                <VideoItem component={"Livestream"} video={video} key={index} />
              ))}
            </div>
          )}
